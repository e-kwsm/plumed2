...
void EffectiveEnergyDrift::update() {
  ...
  if(pDdStep<atoms.getDdStep() && nLocalAtoms<atoms.getNatoms()) {
    ...
    //share stored data
    plumed.comm.Allgatherv((!indexS.empty()?&indexS[0]:nullptr), pNLocalAtoms, &indexR[0], &indexCnt[0], &indexDsp[0]);
    plumed.comm.Allgatherv((!dataS.empty()?&dataS[0]:nullptr), pNLocalAtoms*6, &dataR[0], &dataCnt[0], &dataDsp[0]);
